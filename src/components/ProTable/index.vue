	...
	<div class="table-box">
		...
		>
			<template v-for="item in tableColumns" :key="item">
				...
				<el-table-column v-if="item.type == 'expand'" :type="item.type" :label="item.label" :width="item.width" :fixed="item.fixed" v-slot="scope">
...
import { filterEnum, defaultFormat } from "@/utils";
...
// 表格操作 Hooks
const { tableData, pageable, searchParam, initSearchParam, getTableList, search, reset, handleSizeChange, handleCurrentChange } = useTable(
	props.requestApi,
	props.initParam,
	props.pagination
);
...
.table-box {
	...
	.el-table {
		...
		// 解决 keep-alive || 浏览器窗口大小变化 导致 el-table 样式错乱问题（2.1.4会出现问题）
